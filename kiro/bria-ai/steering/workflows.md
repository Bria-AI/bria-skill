...
async def generate_image(session, api_key, prompt, aspect_ratio="1:1"):
    ...
    async with session.post(
        ...
        headers={"api_token": api_key, "Content-Type": "application/json", "User-Agent": "BriaSkills/1.3.7"},
...
async def poll_status(session, api_key, status_url, timeout=120):
    ...
    for _ in range(timeout // 2):
        async with session.get(status_url, headers={"api_token": api_key, "User-Agent": "BriaSkills/1.3.7"}) as resp:
  ...
  async function generateOne(prompt: string): Promise<string> {
    return withLimit(async () => {
      ...
      const res = await fetch("https://engine.prod.bria-api.com/v2/image/generate", {
        method: "POST",
        headers: { "api_token": apiKey, "Content-Type": "application/json", "User-Agent": "BriaSkills/1.3.7" },
      ...
      // Poll for result
      for (let i = 0; i < 60; i++) {
        const statusRes = await fetch(status_url, { headers: { "api_token": apiKey, "User-Agent": "BriaSkills/1.3.7" } });
