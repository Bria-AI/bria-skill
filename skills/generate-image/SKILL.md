---
name: generate-image
description: Generate images from text descriptions. Powered by Bria FIBO model — commercially-safe, high quality. Hero images, product shots, illustrations, banners, social media.
license: MIT
metadata:
  author: Bria AI
  version: "1.0.0"
...
# Generate Image — Text to Image with AI

Generate high-quality images from text descriptions using Bria's FIBO model. Commercially safe and production ready.
...
HTTP_CODE=$(curl -s -o /tmp/bria_result.json -w "%{http_code}" -X POST \
  ...
  -d '{"prompt": "your description here", "aspect_ratio": "16:9", "resolution": "1MP", "sync": true}')
...
- `aspect_ratio` (optional) — 1:1, 16:9, 4:3, 9:16, 3:4
- `resolution` (optional) — 1MP (default) or 4MP
- `sync` (optional) — true for immediate result
...
**Response:** Returns JSON with `result` array containing `urls` for generated images.
