...
Replace `IMAGE_PATH_OR_URL` with the image URL or local file path. Replace `EDIT_INSTRUCTION` with the natural language edit instruction.

```bash
source ~/.agents/skills/bria-ai/references/code-examples/bria_api.sh
RESULT_URL=$(bria_call /v2/image/edit "IMAGE_PATH_OR_URL" --key images '"instruction": "EDIT_INSTRUCTION"')
echo "$RESULT_URL"
```

**Parameters:**
- `images` (required) — Array with one image URL or local file path (the `--key images` flag handles this)
- `instruction` (required) — Natural language edit instruction (e.g. "change the mug to red", "add sunglasses")

If `bria_call` succeeds, `RESULT_URL` contains the edited image URL — show it to the user.
